...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
	...
	private func prepareVisionRequest() {
		...
		let faceDetectionRequest = VNDetectFaceRectanglesRequest(completionHandler: { request, error in
			...
			guard let request = request as? VNDetectFaceRectanglesRequest, let results = request.results as? [VNFaceObservation] else { return }
	...
	public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
		...
		if trackingRequests.isEmpty {
			...
			do { try VNImageRequestHandler(cvPixelBuffer: pixelBuffer, orientation: orientation, options: handler).perform(detectionRequests)
			} catch let error as NSError { NSLog("Request the error: %@", error) }
			return
		}
		
		do { try sequenceRequestHandler.perform(trackingRequests, on: pixelBuffer, orientation: orientation)
		} catch let error as NSError { NSLog("sequenceRequestHandler Failed: %@", error) }
	...
	func landmarkDetection(pixelBuffer: CVImageBuffer, orientation: CGImagePropertyOrientation, handler: [VNImageOption: AnyObject]) {
		...
		for r in trackingRequests {
			...
			let request = VNDetectFaceLandmarksRequest(completionHandler: { request, error in
				guard let landmarksRequest = request as? VNDetectFaceLandmarksRequest, let results = landmarksRequest.results as? [VNFaceObservation] else { return }
				for face in results {
			...
			} catch { print("Failed Face Landmark Request") }
		}
